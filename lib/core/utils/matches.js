...
function convertAttributes(atts) {
  ...
  return atts.map(att => {
    ...
    switch (att.operator) {
      case '/=':
        let regexFlagsRegex = /^\/(.*)\/([a-z])$/;
        let regexFlagsMatchObj = regexFlagsRegex.exec(attributeValue);
        if (regexFlagsMatchObj) {
          let flags = regexFlagsMatchObj[2];
          let everythingButTheFlags = regexFlagsMatchObj[1];
          regexp = new RegExp(everythingButTheFlags, flags);
        } else {
          regexp = new RegExp(attributeValue);
        }
        break;
      case '^=':
